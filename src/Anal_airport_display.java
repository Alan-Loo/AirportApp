...
public class Anal_airport_display extends JFrame {
    private String[] col_names = {"Airport Name","City","Country"};
    ...
    public Anal_airport_display(ArrayList<String[]> data){
    	this.ddata = data;
    	int num_rows = 0;
    	if (ddata.size() > 0) {
    		num_rows = ddata.size();
    	}
    	String[][] fdArr = new String[1][3];
    	if (num_rows > 0) {
    		fdArr = new String[num_rows][3]; 
    	}   		
    	for (int i = 0; i < num_rows; i++) {
    		fdArr[i][0] = ddata.get(i)[0];
    		fdArr[i][1] = ddata.get(i)[1];	
    		fdArr[i][2] = ddata.get(i)[2];	
    	}
        ...
        setJTableColumnsWidth(dest_table,800,33,33,33);
